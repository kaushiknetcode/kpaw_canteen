...
// Every page is dynamic (fresh CSRF token, session-dependent content),
// so nothing here should ever be cached — not by the browser, not by a
// CDN, and not by a mobile carrier's network-level caching proxy (common
// on Indian mobile networks). Without this, a cached stale page can
// silently serve someone an expired CSRF token, breaking their form
// submission for a reason that has nothing to do with them being slow.
header('Cache-Control: no-store, no-cache, must-revalidate, private, max-age=0');
header('Pragma: no-cache');
header('Expires: 0');

//require __DIR__ . '/../offline.html'; exit;
