...
<?php if (!empty($errors) && empty($fieldErrors)): ?>
    ...
    <?php endforeach; ?>
<?php endif; ?>
<form method="post" novalidate id="kpawOtpForm">
    <?= csrf_field() ?>
    <div class="mb-3">
        <input type="text" name="otp" id="kpawOtp"
               class="<?= 'form-control text-center' . (isset($fieldErrors['otp']) ? ' is-invalid' : '') ?>"
               maxlength="6" pattern="[0-9]{6}" inputmode="numeric" autocomplete="one-time-code" required autofocus>
        <div class="invalid-feedback text-center">Invalid or expired code. Please try again or request a new one.</div>
    </div>
...
<script>
    document.getElementById('kpawOtp').addEventListener('input', function (e) {
