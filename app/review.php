...
// ------------------------------------------------------------
// POST: validate the cart, store it in session, then REDIRECT to this
// same page as a GET request (Post-Redirect-Get). This is what stops
// the browser's "Confirm Form Resubmission" warning on back — without
// it, hitting Back would try to re-POST the cart data instead of just
// re-displaying a page, which is exactly the error you saw.
// ------------------------------------------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Re-validate every item server-side — never trust quantities or
    // prices from the client. Only real, active items belonging to this
    // exact canteen + meal_type make it into the validated cart.
    $stmt = $pdo->prepare(
    ...
    header('Location: /app/review.php');
    exit;
}

// ------------------------------------------------------------
// GET: render from session only. Reachable either right after the
// redirect above, or by hitting Back/refresh — always safe, since
// GET never resubmits anything.
// ------------------------------------------------------------
...
<div class="card mb-3">
    <div class="card-body">
        ...
        <p class="mb-1 small text-muted">
            <?= htmlspecialchars(format_date_with_day($pending['target_date'])) ?> &middot;
...
<table class="table">
    ...
    <tfoot>
        <tr class="fw-bold"><td colspan="2">Total</td><td class="text-end">&#8377;<?= number_format($pending['total'], 2) ?></td></tr>
    </tfoot>
</table>

<a href="/app/payment.php" class="btn btn-primary w-100">Pay Now</a>
<a href="/app/book.php?<?= htmlspecialchars(http_build_query(['meal_type' => $pending['meal_type'], 'date' => $pending['target_date'], 'canteen_id' => $pending['canteen_id']])) ?>" class="btn btn-link w-100">Edit Order</a>
